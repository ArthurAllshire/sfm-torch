...
def test_conv_net_shapes():
    ...
    out, conv = net(image)
    assert out.size() == (4, 32, 384, 128), "SFM Conv net primary output shape does not match"
    assert conv.size() == (4, 1024, 12, 4), "SFM Conv net conv output shape does not match"
